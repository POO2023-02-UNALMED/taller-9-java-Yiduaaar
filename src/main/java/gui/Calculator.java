...
public class Calculator extends VBox implements EventHandler<ActionEvent>{
	...
	public void handle(ActionEvent event) {
		...
		if (value.matches("[0-9]")) {
			// Si el valor es un número, se agrega al número actual en función del operador
			if (operator == null) {
				number1 += value;
			} else {
				number2 += value;
			...
			// Si el valor es un operador, se guarda el operador actual
			operator = value;
		} else if (value.equals("=")) {
			// Si el valor es "=", se realiza la operación y se muestra el resultado en el display
			double result = 0;
        
			if (operator != null && !number1.equals("") && !number2.equals("")) {
				...
				switch (operator) {
					...
					case "/":
						if (num2 != 0) {
							...
							result = Double.NaN;  // División por cero, resultado indeterminado
						}
						break;
				}
			}
			...
			// Si el valor es "C", se reinicia la calculadora
			number1 = "";
			number2 = "";
			operator = null;
			displayText.setText("");
